...
void solve2() // tabulation format ---> optimized space complexity
{
    string a, b;
    cin >> a >> b;

    int n = a.size(), m = b.size();
    vector<int> prev(m + 1, 0), curr(m + 1, 0); // both of m size
    ...
    cout << prev[m] << endl; // returns --> prev --> m
}
